...
class application : public nextgen::singleton<application>
{
    ...
    private: struct variables
    {
        ...
        proxyprobe::proxy_checker proxy_checker;
...
void application::run(int argc, char* argv[])
{
    ...
    self->proxy_database.connect("localhost", "root", "", "proxies");
    ...
    while(true)
    {
        if(timer.stop() > 10)
        {
            std::cout << "[proxyprobe:application:run] Updating services..." << std::endl;
