#ifndef PROXYPROBE_PROXY_CHECKER
#define PROXYPROBE_PROXY_CHECKER
...
bool PROXYPROBE_DEBUG_1 = 1;

namespace proxyprobe
{
    class proxy_checker
    {
        class job_type
        ...
        public: void initialize()
        {
            ...
            [=](client_type client)
            {
                if(PROXYPROBE_DEBUG_1)
                    std::cout << "[proxyprobe:proxy_server] Proxy server (port 8080) accepted HTTP client." << std::endl;

                client.receive(
                [=](message_type r1)
                {
                    if(PROXYPROBE_DEBUG_1)
                        std::cout << "[proxyprobe:proxy_server] Received data from HTTP client." << std::endl;
                    //  can use cookies, can
                    ...
                    if(r1->header_list.find("pid") != r1->header_list.end())
                    {
                        ...
                        // check if job wasnt removed because proxy server killed the connection before sending out packets
                        {
                            ...
                            if(PROXYPROBE_DEBUG_1)
                                std::cout << "[proxyprobe:proxy_server] Server sending response to " << proxy->host << ":" << proxy->port << std::endl;

                            client.send(r2,
                            [=]
                            {
                                if(PROXYPROBE_DEBUG_1)
                                    std::cout << "[proxyprobe:proxy_server] Server response successful to " << proxy->host << proxy->port << std::endl;
                            ...
                            {
                                if(PROXYPROBE_DEBUG_1)
                                    std::cout << "[proxyprobe:proxy_server] Server response failure to " << proxy->host << proxy->port << std::endl;
                        ...
                        {
                            if(PROXYPROBE_DEBUG_1)
                                std::cout << "[proxyprobe:proxy_server] got confirmation" << std::endl;
                        ...
                        {
                            if(PROXYPROBE_DEBUG_1)
                                std::cout << "[proxyprobe:proxy_server] GAY PROXY" << std::endl;
                ...
                [=]
                {
                    std::cout << "[proxyprobe:proxy_server] Failed to receive data from HTTP client." << std::endl;
                });
            },
            [=]
            {
                std::cout << "[proxyprobe:proxy_server] Failed to accept HTTP client." << std::endl;
            });
        ...
        private: void test_connection() const
        {
            ...
            if(PROXYPROBE_DEBUG_1)
                std::cout << "[proxyprobe:proxy_checker] Testing connection." << std::endl;
            ...
            {
                if(PROXYPROBE_DEBUG_1)
                    std::cout << "[proxyprobe:proxy_checker] Connected to self." << std::endl;
        ...
        private: void connect(job_type job) const
        {
            ...
            if(PROXYPROBE_DEBUG_1)
                std::cout << "[proxyprobe:proxy_client] Attempting to connect to " << proxy->host << ":" << proxy->port << " (" << proxy->id << ")" << std::endl;
            ...
            {
                if(PROXYPROBE_DEBUG_1)
                    std::cout << "[proxyprobe:proxy_client] Connected to proxy " << proxy->host + ":" + nextgen::to_string(proxy->port) << "." << std::endl;
                ...
                [=](message_type r2)
                {
                    if(PROXYPROBE_DEBUG_1)
                    {
                        std::cout << "[proxyprobe:proxy_client] Client send/receive successful." << std::endl;
                ...
                {
                    if(PROXYPROBE_DEBUG_1)
                        std::cout << "[proxyprobe:proxy_client] Client send/receive failure." << std::endl;
